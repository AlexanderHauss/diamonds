	...
	var $div = document.getElementById("hire-container");
	...
	var steps_total = Math.round(transition_time * 200); // total amount of steps
	var rgb_steps = {
	  start: [0, 0, 0],
	  stop: [0, 0, 0]
	...
	function updateGradient() {
	  ...
	  if (t_color1 != color1 || t_color2 != color2) {
		...
		for (var i = 0; i < 4; i++) {
		  div_style.backgroundImage =
			prefixes[i] + "linear-gradient(45deg, " + color1 + ", " + color2 + ")";
		}
	  }
...
$('#submit').click(function(){
	$.post("../library/send-work.php", $("#form-contatto").serialize(),  function(response) {   
